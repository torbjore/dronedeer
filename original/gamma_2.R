...
DoubleObsMultisiteConf <- configureMCMC(DoubleObsMultisiteModel, monitors = c("mean_lambda" , "p1", "p2", "mu0", "sigma", "beta", "x_at_peak"), enableWAIC = TRUE)
DoubleObsMultisiteMCMC <- buildMCMC(DoubleObsMultisiteConf)
CDoubleObsMultisiteMCMC <- compileNimble(DoubleObsMultisiteMCMC)

posterior_gamma_2 <- runMCMC(
...
plot(posterior_gamma_2$samples)
summary(posterior_gamma_2$samples)
posterior_gamma_2$WAIC

save(posterior_gamma_2, file = "Manuscript_models/posterior_gamma_2.RData")


# toppunkt: x = - b/2c
x_at_peak = - unlist(posterior_gamma_2$samples[,"beta[1]"])/(2*unlist(posterior_gamma_2$samples[,"beta[2]"]))
plot(density(x_at_peak))
mean(x_at_peak)

x_at_peak_in_meters = mean(mean(UseData$mean_field_dist) + x_at_peak * sd(UseData$mean_field_dist)) 
mean(UseData$mean_field_dist) + mean(x_at_peak) * sd(UseData$mean_field_dist)
