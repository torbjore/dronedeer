...
(HMM... JEG ER IKKE HELT SIKKER PÅ FORKLARINGEN HER - KANSKJE BRUK EN GAMMA-PRIOR ISTEDENFOR?) For the log-normal and the gamma models, the prior distribution for $\sigma$ was specified as $\text{Uniform}(0.5,3)$. The lower bound of $0.5$ was set to prevent poor mixing when the mcmc-chains occasionally entered the lower tail of the posterior distribution. The chains never got close to the upper bound of $3$.
