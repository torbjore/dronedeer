...
# RESULTS ARE VERY SIMILAR TO THE MODEL I ALLREADY HAVE IN THE SUPPL INFO, SO DON'T USE THIS
...
DoubleObsMultisiteCode_fence <- nimbleCode({
  ...
  for(s in 1:N_surv){
    logit_p1[s] ~ dnorm(eta[s], sd = sigma_p[s])
    logit_p2[s] ~ dnorm(eta[s], sd = sigma_p[s])
