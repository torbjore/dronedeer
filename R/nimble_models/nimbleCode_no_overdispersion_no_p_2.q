# Model with NO OVERDISPERSION AND PERFECT DETECTION and second order covariate effect
...
nimbleCode_DOMM_gamma_2 <- nimbleCode({ # Double observer multi-site model (DOMM)
  ...
  for(s in 1:N_surv){
    for(i in 1:N_sites[s]){
      ...
      lambda[s,i] <- exp(mu0[sam[s]] + x[s,i]*beta[1] + x[s,i]^2*beta[2])
    }
  }
  ...
  for(s in 1:N_surv){
    for(i in 1:N_sites[s]){
      # New_N[s,i] ~ dpois(lambda[s,i]*area[s,i])
      # New_Y[s,i] ~ dbin(Psum[s,i]-0.01, New_N[s,i])
      New_Y[s,i] ~ dpois(lambda[s,i]*area[s,i]) # NB! Only when Psum[s,i] = 1
      # New_y[s,i,1:3] ~ dmulti(pi[s,i,1:3], Y[s,i])
      #E_Y[s,i] <- Psum[s,i]*N[s,i] + 0.0001
      E_Y[s,i] <- Psum[s,i]*lambda[s,i]*area[s,i] + 0.0001
