...
source("R/nimble_models/enclosure_model_p.q")
...
phat = (p1hat+p2hat)/2
Nhat = apply(nimbleData_Fence$data$Y, 1, sum, na.rm=TRUE)/(1-(1-p1hat)*(1-p2hat)) # For all sites combined
lambdahat = (Nhat/nimbleData_Fence$constants$N_sites)/apply(nimbleData_Fence$constants$area, 1, mean, na.rm=TRUE) + 0.01 # Adding a small value since we get -Inf from log(lambdahat=0)
...
Inits = function(){
  ...
  list(
    ...
    epsilon = matrix(rnorm(nrow(nimbleData_Fence$data$Y)*ncol(nimbleData_Fence$data$Y), 0, sigma) , nrow = nrow(nimbleData_Fence$data$Y), ncol = ncol(nimbleData_Fence$data$Y)),
    eta = runif(2, -1, 1),
...
#save(posterior_samples, file = "data/posterior_samples/posterior_samples_enclosure_p.RData")