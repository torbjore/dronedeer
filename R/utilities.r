...
pred_plots <- function(predfun = pred, x=X, x_st=X_st, PS, names = sam_names){
  ...
  for(SAM in 1:length(sam_names)){
    ...
    plots[[SAM]] = ggplot(data = df, aes(x=x)) + 
      ...
      #theme_ipsum(axis_title_size = 11.5, axis_title_just = "cc",) # TE>Tar bort dette for jeg har ikke alle fontene som kreves installert på min maskin ser det ut til
